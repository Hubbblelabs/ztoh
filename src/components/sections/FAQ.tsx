                        ...
                        {faqs
                            ...
                            .map((faq, index) => (
                                ...
                                >
                                    <button
                                        onClick={() =>
                                            setOpenIndex(openIndex === index * 2 ? null : index * 2)
                                        }
                                    ...
                                    >
                                        ...
                                        <div
                                            className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-colors ${openIndex === index * 2 ? 'bg-primary text-white' : 'bg-slate-100 text-slate-500'}`}
                                        >
                                            {openIndex === index * 2 ? (
                                                <Minus size={16} />
                                            ) : (
                            ...
                            .map((faq, index) => (
                                ...
                                >
                                    <button
                                        onClick={() =>
                                            setOpenIndex(
                                                openIndex === index * 2 + 1 ? null : index * 2 + 1,
                                            )
                                        }
                                        className="w-full flex items-center justify-between p-6 text-left hover:bg-slate-50 transition-colors"
                                    >
                                        ...
                                        <div
                                            className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-colors ${openIndex === index * 2 + 1 ? 'bg-primary text-white' : 'bg-slate-100 text-slate-500'}`}
                                        >
                                            {openIndex === index * 2 + 1 ? (
                                                <Minus size={16} />
                                            ) : (
                                    ...
                                    <motion.div
                                        initial={false}
                                        animate={{
                                            height: openIndex === index * 2 + 1 ? 'auto' : 0,
                                        }}
                                        className="overflow-hidden"
                                    >
                                        <div className="p-6 pt-0 text-slate-600 leading-relaxed border-t border-slate-50">
