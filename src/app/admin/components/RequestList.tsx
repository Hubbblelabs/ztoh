...
export default function RequestList({ activeTab, showToast }: RequestListProps) {
    ...
    const [selectedRequest, setSelectedRequest] = useState<JoinRequest | ContactRequest | null>(
        null,
    );
    ...
    const [statusFilter, setStatusFilter] = useState<'all' | 'pending' | 'accepted' | 'declined'>(
        'all',
    );
    ...
    const handleDeleteRequest = async () => {
        ...
        try {
            const res = await fetch(
                `/api/admin/requests/${selectedRequest._id}?type=${activeTab}`,
                {
                    method: 'DELETE',
                },
            );
            ...
            <div className="flex flex-col sm:flex-row justify-between gap-4">
                <div className="flex flex-wrap gap-3">
                    ...
                    <Select
                        value={statusFilter}
                        onValueChange={(value) =>
                            setStatusFilter(value as 'all' | 'pending' | 'accepted' | 'declined')
                        }
                    >
                    ...
                    {activeTab === 'join' && (
                        <Select
                            value={filterType}
                            onValueChange={(value) =>
                                setFilterType(value as 'all' | 'student' | 'teacher')
                            }
                        >
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {activeTab === 'join' &&
                                                        (request as JoinRequest).attachments &&
                                                        (request as JoinRequest).attachments!
                                                            .length > 0 && (
                                                            <span
                                                                className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-50 text-blue-600 rounded-full text-[10px] font-semibold"
                                                                title={`${(request as JoinRequest).attachments!.length} attachment(s)`}
                                                            >
                                                                <Paperclip size={10} />
                                                                {
                                                                    (request as JoinRequest)
                                                                        .attachments!.length
                                                                }
                                                            </span>
                                                        )}
                                            ...
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                {activeTab === 'join' &&
                                                    ...
                                                    (request as JoinRequest).attachments!.length >
                                                        0 && (
                                                        <span
                                                            className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-blue-50 text-blue-600 rounded-full text-[9px] font-semibold"
                                                            title={`${(request as JoinRequest).attachments!.length} attachment(s)`}
                                                        >
                                                            <Paperclip size={9} />
                                                            {
                                        ...
                                        {activeTab === 'join' && (
                                            <span
                                                className={`px-2 py-0.5 rounded-full font-bold uppercase tracking-wide ${
                                                    (request as JoinRequest).type === 'teacher'
                                                        ? 'bg-purple-100 text-purple-700'
                                                        : 'bg-blue-100 text-blue-700'
                                                }`}
                                            >
                    ...
                    <div className="bg-card rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                        <div className="p-6 border-b border-border flex justify-between items-center sticky top-0 bg-card z-10">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                    Manage status and track history
                                </p>
                            </div>
                            <button
                        ...
                        <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="lg:col-span-2 space-y-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    {Object.entries(selectedRequest).map(([key, value]) => {
                                        ...
                                        return (
                                            <div
                                                key={key}
                                                className="grid grid-cols-3 gap-4 pb-4 border-b border-border last:border-0"
                                            >
                                                <span className="text-sm font-semibold text-muted-foreground capitalize">
                                ...
                                {/* Attachments */}
                                {activeTab === 'join' &&
                                    (selectedRequest as JoinRequest).type === 'teacher' && (
                                        <div className="space-y-4 pt-4 border-t border-border">
                                            <h4 className="font-bold text-foreground">
                                                Attachments
                                            </h4>
                                            {(selectedRequest as JoinRequest).attachments &&
                                            (selectedRequest as JoinRequest).attachments!.length >
                                                0 ? (
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                    {(
                                                        selectedRequest as JoinRequest
                                    ...
                                    <div className="bg-muted rounded-md p-4 border border-border max-h-60 overflow-y-auto mb-4 space-y-3">
                                        {selectedRequest.notes?.length ? (
                                            selectedRequest.notes.map((note, idx) => (
                                                <div
                                                    key={idx}
                                                    className="bg-card p-3 rounded-md border border-border shadow-sm"
                                                >
